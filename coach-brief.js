...
function paintCoachBrief(c,result,pending) {
  ...
  $('coachBrief').innerHTML=`<article class="panel"><div class="panel-title"><div><span class="eyebrow">${result?.source==='ai'?'AI Coach Brief':'Automated Coach Summary'}</span><h2>${heading}</h2></div><button class="secondary small" id="coachRefresh" ${pending?'disabled':''}>${pending?coachText('Анализируем…','Analyzing…'):coachText('Обновить анализ','Refresh analysis')}</button></div>
    ...
    <small class="muted">${pending?coachText('Автоматическая сводка уже готова. AI уточняет приоритет.','Automated summary is ready. AI is refining the priority.'):result?.source==='ai'?coachText('AI выбрал факты; числа и цели рассчитаны приложением.','AI selected evidence; the application calculated all numbers and targets.'):coachText('ИИ-тренер временно недоступен. Показан автоматический анализ на основе ваших данных.','AI Coach temporarily unavailable. Automated analysis is shown instead.')}</small></article>
