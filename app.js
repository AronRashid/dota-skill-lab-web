...
const I18N = {
  ru: {
    brandSubtitle:'Web Beta · Final', navOverview:'Обзор', navCoach:'Тренер', navMatches:'Матчи', navPatterns:'Паттерны', navSessions:'Сессии', navHeroes:'Герои', navMistakes:'Ошибки', navTraining:'Тренировка', navProgress:'Прогресс',
  ...
  en: {
    brandSubtitle:'Web Beta · Final', navOverview:'Overview', navCoach:'Coach', navMatches:'Matches', navPatterns:'Patterns', navSessions:'Sessions', navHeroes:'Heroes', navMistakes:'Mistakes', navTraining:'Training', navProgress:'Progress',
...
function renderOverview(){
  ...
  renderDNA();renderFocus();renderTrend();renderRoleMix();renderScoreBars();renderTopHeroes();renderCoachCockpit();renderQuickCompare();renderInsightSummary();renderCoachBrief();
}
...
function renderTraining(){
  ...
  $('trainingBody').innerHTML=`<div class="quest-hero"><div class="quest-ring" style="--quest:${active?Math.round(results.length/5*100):0}"><b>${results.length}/5</b><span>${t('questProgress')}</span></div><div class="quest-copy"><span class="quest-status ${results.length>=5&&passed>=4?'mastered':results.length>=5?'repeat':active?'ontrack':'ready'}">${status}</span><h3>${safeText(coachTargetLabel(tg))}</h3><p>${t('trainingFocus')}: <b>${roleName(tg.role)}</b> · ${t('nextFiveRole')}</p><div class="quest-run">${[0,1,2,3,4].map(i=>{const x=results[i];return`<div class="quest-game ${x?(x.pass?'pass':'fail'):''}"><i>${x?(x.pass?'✓':'×'):i+1}</i><span>${x?`${safeText(heroName(x.m.hero_id))}<small>${x.m.kills}/${x.m.deaths}/${x.m.assists}</small>`:`${t('gameWord')} ${i+1}`}</span></div>`}).join('')}</div>${active?`<div class="training-rule">${t('completed')}: <b>${passed}/5</b>${results.length>=5?` · <b>${passed>=4?t('blockFixed'):t('repeatBlock')}</b>`:''}</div>`:`<div class="training-rule">${t('startBlockHint')}</div>`}</div></div>`;
  $('startTrainingBtn').classList.toggle('hidden',!!active);$('resetTrainingBtn').classList.toggle('hidden',!active);
}
function startTraining(){ const candidate=buildCoachContext().priorities[0];if(candidate)startCoachCycle(candidate); }
function resetTraining(){ localStorage.removeItem(trainingStorageKey());queueRemoteUserStateSync();renderTraining();renderCoachCockpit();renderCoachBrief(); }
